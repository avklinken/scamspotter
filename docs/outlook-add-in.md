...
De huidige add-in gebruikt een add-in-only XML-manifest, omdat het Microsoft 365 unified manifest niet wordt ondersteund in Outlook op Mac. De `VersionOverrides`-sectie definieert de knop in Message Read; de manifestversie is verhoogd naar `1.0.4.0` en gebruikt PNG-iconen die door Outlook worden ondersteund.
